...
export const CommitsGrid = ({ text }) => {
  ...
  const generateHighlightedCells = (textVal) => {
    const cleanedText = cleanString(textVal);
  ...
  return (
    <section
      className="w-full bg-[#070b19]/25 border border-white/5 grid p-3 gap-1 rounded-xl"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              "border border-white/[0.02] h-full w-full aspect-square rounded-[3px]",
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-white/[0.01]" : ""
            )}
            style={{
